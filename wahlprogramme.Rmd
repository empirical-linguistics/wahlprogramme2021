...
![Wortwolken der einzelnen Parteiprogramme](wordclouds01.png)
...
![Wortwolken der Trigramme](trigram_wcs02a.png)

... und anschließend nur mit jenen, die keine Stopwords enthalten:

```{r wctripub2, eval = FALSE}
...
![Wortwolken der Trigramme (ohne Stopwords)](trigram_wcs02b.png)
...
```{r tfidf, fig.height=12}

tri_count %>% bind_tf_idf(term = trigram, document = Partei, n = Freq) %>% select(Partei, trigram, tf, idf, tf_idf) %>% arrange(desc(tf_idf)) %>% datatable() %>% 
...
tri_count %>% filter(in_stopwords == 0) %>% bind_tf_idf(term = trigram, document = Partei, n = Freq) %>% select(Partei, trigram, tf, idf, tf_idf) %>% arrange(desc(tf_idf)) %>% datatable() %>% 
  formatSignif(columns = c("tf", "idf",  "tf_idf"), digits=3)
...
Was unterschiedliche rhetorische Strategien angeht, wird beispielsweise deutlich, dass die SPD sich vor allem auf 
